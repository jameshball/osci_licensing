...
public:
    LicenseAndUpdatesComponent (LicenseManager& licenseManagerToUse, ProductUpdateConfig configToUse)
        : licenseManager (licenseManagerToUse),
          config (std::move (configToUse)),
          licenseCard (config.productIcon, config.copyIconSvg, config.revealIconSvg, config.concealIconSvg),
          helpButton ("accountHelp", config.helpIconSvg, juce::Colours::white) {
...
private:
    ...
    void refreshState() {
        const auto state = licenseManager.getStateForUi();
        const auto status = state.getProperty ("status").toString();
        const auto premium = static_cast<bool> (state.getProperty ("premium"));
        const auto premiumRequired = requiresPremiumLicense();
        const auto betaEnabled = UpdateSettings (config.productSlug).betaUpdatesEnabled();
        ...
        licenseState.productName = premium ? juce::String() : config.productName;
        licenseState.title = premium ? config.productName
        ...
        updateState.currentVersion = config.currentVersion;
    ...
    juce::String detailTextForState (const juce::ValueTree& state, bool premium, bool premiumRequired) const {
        if (premium) {
        ...
        return config.compiledVariant == "premium" ? "This build is usable without activation."
                                                    : "No premium license activated.";
    }

    juce::String badgeTextForState (const juce::String& status, bool premium, bool premiumRequired) const {
        ...
        return config.compiledVariant == "premium" ? "Unlicensed" : "Free";
    }
    ...
    juce::String unlicensedTitle() const {
        return config.compiledVariant == "premium" ? "License not activated" : "Free version";
    }

    bool hasFreeFallback() const {
        return config.hasFreeFallback;
    }
    ...
    void refreshCachedLicenseIfNeeded() {
        ...
        const auto status = licenseManager.status();
    ...
    void confirmDeactivateLicense() {
        ...
        options.title = "Remove License";
        options.message = "This removes the locally cached license from this computer. You can activate again later with your license key.";
        options.icon = ErrorOverlay::Icon::Warning;
        options.preferredPanelSize = { 500, 270 };
        options.buttons = {
            { "Remove", [safeThis] {
                if (safeThis != nullptr) {
        ...
        ErrorOverlay::show (*this, std::move (options));
    }
    ...
    void disableBetaUpdates() {
        UpdateSettings (config.productSlug).useStableTrack();
        availableVersion.reset();
        downloadedFile = juce::File();
        setNotice (NoticeTarget::Update, "Stable updates enabled.", NoticeKind::Success);

        if (config.onUpdateSettingsChanged) {
            config.onUpdateSettingsChanged();
        }
    ...
    void checkForUpdates() {
        ...
        const auto product = config.productSlug;
        const auto currentVersion = config.currentVersion;
    ...
    void downloadAndInstallUpdate() {
        ...
        downloadAndInstallVersion (*availableVersion, licenseManager.getCachedToken(), NoticeTarget::Update);
    }
    ...
    void installUpdate (NoticeTarget noticeTarget) {
        ...
        osci::InstallPrompt::showConfirmation ({
            ...
            [safeThis, file, version, product, currentVersion, noticeTarget] {
                ...
                request.manifest = safeThis->config.linuxInstallManifest;
                request.iconPng = safeThis->config.linuxIconPng;
    ...
    osci::ReleaseTrack selectedReleaseTrack() const {
        return UpdateSettings (config.productSlug).releaseTrack();
    }
