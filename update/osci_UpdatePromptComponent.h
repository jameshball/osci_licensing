...
public:
    UpdatePromptComponent(LicenseManager& licenseManagerToUse, ProductUpdateConfig configToUse)
        : licenseManager(licenseManagerToUse), config(std::move(configToUse)),
          closeButton("Dismiss update", Colours::text(), Colours::text()) {
        setVisible (false);
        setAlwaysOnTop (true);
    ...
    bool showPendingInstallStatusIfNeeded() {
        PendingInstall pending (config.productSlug);
        auto marker = pending.load();
        if (! marker.has_value()) {
            return false;
        }

        if (PendingInstall::isResolvedByRunningVersion (*marker, config.currentVersion)) {
            pending.clear();
...
private:
    ...
    public:
        ...
        void paint (juce::Graphics& g) override {
            ...
            g.setColour (Colours::veryDark().brighter (0.12f));
            g.fillRoundedRectangle (bounds, 8.0f);

            if (fillWidth > 1.0f) {
                g.setColour (Colours::accentColor().withAlpha (0.92f));
    ...
    void dismissFor48Hours() {
        if (availableVersion.has_value()) {
            UpdateSettings(config.productSlug).dismiss (availableVersion->semver, currentTimeSeconds());
        }
    ...
    bool isDismissed (const juce::String& semver) const {
        return UpdateSettings(config.productSlug).isDismissed (semver, currentTimeSeconds());
    }
    ...
    juce::String updateAvailableDetailText (const osci::VersionInfo& version) const {
        if (isPremiumDownloadWithoutToken (version, licenseManager.getCachedToken())) {
