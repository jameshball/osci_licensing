...
Downloader::Downloader (Config configToUse) : config (std::move (configToUse)), backend (config.backend) {
...
juce::Result Downloader::downloadAndVerify (const VersionInfo& version,
    ...
    const juce::int64 reportedSize = version.sizeBytes > 0 ? version.sizeBytes : 0;
    const juce::int64 hintedSizeCap = reportedSize + reportedSize / 2;
    const juce::int64 sizeCap = (reportedSize > 0 && hintedSizeCap < absoluteSizeCap)
        ? hintedSizeCap
        : absoluteSizeCap;
